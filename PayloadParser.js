//Test: 01 69 01 0E10 00 0000000000  - Tiene que dar 3600s y no warning 0
...
function parseUplink(device, payload) {
    ...
    // Store Hearbeattime
    if (decoded.hearbeattime != null) {
        var sensor2 = device.endpoints.byAddress("2");

        if (sensor2 != null)
            sensor2.updateGenericSensorStatus(decoded.hearbeattime);
    };

    // Store Warning Status
    if (decoded.warning != null) {
        var sensor3 = device.endpoints.byAddress("3");

        if (sensor3 != null)
            sensor3.updateIASSensorStatus(decoded.warning);
    };
...
function Decoder(bytes, fport) {
    ...
    if (fport === 6) { // then its ReportDataCmd
        if (bytes[2] === 0x00) { // version report
            ...
            decoded.devicetype = "R602A";
            decoded.battery = bytes[6] / 10;
            decoded.hearbeattime = ((bytes[3] << 8) + bytes[4]);
            decoded.warning = bytes[5];
            return decoded;
        }
